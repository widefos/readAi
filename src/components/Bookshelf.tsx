...
export function Bookshelf({ books, onSelectBook, onUploadClick, onDeleteBook, progress, theme }: BookshelfProps) {
  ...
  const titleColor = { light: 'text-[#111]', paper: 'text-[#1A1A1A]', dark: 'text-[#E3E3E3]' } as const;
  const subtitleColor = { light: 'text-blue-600/65', paper: 'text-indigo-600/60', dark: 'text-sky-300/70' } as const;
  const coverBase = {
  ...
  const fallbackCover = { light: 'bg-[#f8f8fb] border border-black/10', paper: 'bg-[#fdfdfd] border border-black/5', dark: 'bg-[#191919] border border-white/10' } as const;
  const fallbackBar = { light: 'bg-black/10', paper: 'bg-black/5', dark: 'bg-white/10' } as const;
  const fallbackIcon = { light: 'text-black/15', paper: 'text-black/10', dark: 'text-white/20' } as const;
  const fallbackTitle = { light: 'text-black/45', paper: 'text-black/40', dark: 'text-white/55' } as const;
  const progressTrack = { light: 'bg-black/8', paper: 'bg-black/5', dark: 'bg-white/10' } as const;
  const progressFill = { light: 'bg-[#111]', paper: 'bg-black', dark: 'bg-white' } as const;
  const hoverMask = { light: 'group-hover:bg-black/6', paper: 'group-hover:bg-black/5', dark: 'group-hover:bg-white/5' } as const;
  const bookTitleColor = { light: 'group-hover:text-blue-600 text-[#1A1A1A]', paper: 'group-hover:text-indigo-600 text-[#1A1A1A]', dark: 'group-hover:text-sky-300 text-[#DBDBDB]' } as const;
  const authorColor = { light: 'text-black/40', paper: 'text-black/30', dark: 'text-white/35' } as const;
  const dividerColor = { light: 'bg-black/8', paper: 'bg-black/5', dark: 'bg-white/10' } as const;
  const unreadColor = { light: 'text-black/30', paper: 'text-black/20', dark: 'text-white/30' } as const;
  const percentColor = { light: 'text-blue-600', paper: 'text-indigo-500', dark: 'text-sky-300' } as const;
  const menuPanel = { light: 'bg-[#FCFCFD] border border-black/12 shadow-[0_14px_30px_rgba(0,0,0,0.16)]', paper: 'bg-white shadow-[0_14px_30px_rgba(0,0,0,0.12)]', dark: 'bg-[#1B1B1B] border border-white/12 shadow-[0_14px_30px_rgba(0,0,0,0.5)]' } as const;
  const menuButton = { light: 'text-black/70 hover:bg-black/[0.04]', paper: 'text-[#666] hover:bg-[#F7F7F7]', dark: 'text-white/75 hover:bg-white/8' } as const;
  const addButton = { light: 'border-black/12 hover:border-blue-400/40 hover:bg-blue-50/30', paper: 'border-gray-100 hover:border-indigo-200 hover:bg-indigo-50/20', dark: 'border-white/15 hover:border-sky-300/40 hover:bg-white/5' } as const;
  const addIcon = { light: 'text-black/30 group-hover:text-blue-500', paper: 'text-gray-200 group-hover:text-indigo-300', dark: 'text-white/25 group-hover:text-sky-300/70' } as const;
  const addText = { light: 'text-black/35 group-hover:text-blue-600', paper: 'text-gray-300 group-hover:text-indigo-400', dark: 'text-white/35 group-hover:text-sky-300' } as const;
